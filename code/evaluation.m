...
%     xw = addnoise(x,nw(16000+(1:length(x))),10,fs,ref);
    xw = addnoise1(x,nw(16000:end),0,e_vad,ref,Nw,Nsh);
    subplot(212);
    plot(xw,'Color',[.8,.8,.8]);axis tight;
    
    % nbb = read_audio('Noisefiles\babble.wav','wav');
    % xbb = addnoise(x,nbb(16000+(1:length(x))),0,fs);

%     X = periodogram(x,Nw,Nsh,nfft);
%     S = abs(X);%.^2;
%     subplot(312);imagesc(-20*log10(S));set(gca,'YDir','normal');

    % Sbb = periodogram(xbb,Nw,Nsh,nfft);
    % Sbb = 20*log10(abs(Sbb));
%     colormap gray;caxis([-110 -40]);

    
    g729.vad = G729(xw,fs,Nw,Nsh);
%     g729.vad = vadhangover(g729.vad,fs*MIN_NS);
    g729.result = vadperformance(ref(ref_start:end),g729.vad(ref_start:end));
...
%     pp.of = nfft/Nsh; % overlap factor
%     pp.pr = 0.6;      % threshold
    sohn.vad = vadsohn(xw,fs);
%     sohn.vad = vadhangover(sohn.vad,fs*MIN_NS);
    sohn.result = vadperformance(ref(ref_start:end),sohn.vad(ref_start:end));
